...
var emulatorMain = {
  ...
  evaluate: function () {
    ...
    if (curCommands[0] != '') {
            ...
            case '1':
              if (commandStr[2] == '1') {
                if (commandStr.slice(3, 5) == '08' || commandStr.slice(3, 5) == '09') {
                  ...
                  parseInt(commandStr.slice(3, 5)) >= 10 &&
                  parseInt(commandStr.slice(3, 5)) <= 99
                ) {
        ...
        case '2':
          if (commandStr.slice(1, 3) == '0' + (curTpr + 1).toString()) {
            if (commandStr.slice(1, 3) == '0' + (curTpr + 1).toString()) {
              ...
              !between(parseInt(commandStr.slice(3, 5)), 8, 99)
            ) {
              this.$.log.set(
                'value',
                prevText +
                  'Error: you must pick a defined store, or the accumulator. Valid stores are 09-99.',
              )
            }
          } else if (
            parseInt(commandStr.slice(3, 5)) == 1 ||
            parseInt(commandStr.slice(3, 5)) == 3
          ) {
            ...
            if (commandStr.slice(1, 3) != '08' && commandStr.slice(1, 3) != '09') {
              ...
              var printText =
                this.$[commandStr.slice(1, 3)].get('content')[0] +
                this.$[commandStr.slice(1, 3)].get('content').slice(8, 15)
      ...
      if (
        this.$['num' + (curTpr + 1)].get('value').indexOf(/<[^>]+>/gi) != -1 ||
        this.$['num' + (curTpr + 1)].get('value') == ''
      ) {
        this.finishLightOn()
        this.stop()
      }
  ...
  resetTapes: function () {
    ...
    enyo.log(curCommands)
    for (var c = 0; c < 4; c++) {
      tpr[parseInt(this.$['num' + (c + 1)].name.substring(3)) - 1] =
        this.$['num' + (c + 1)].get('value')
    }
